    ...
    if minmax == False:
        for next_angle in range(rot_range[0], rot_range[1], rot_interval):
            for next_scale in range(scale_range[0], scale_range[1], scale_interval):
                ...
                print("Angle" + str(next_angle) + "/t" + "Scale" + str(next_scale))
                if next_angle == 0:
                ...
                if method == "TM_CCOEFF":
                    ...
                    raise RuntimeError("There's no such comparison method for template matching.")
        ...
        for next_angle in range(rot_range[0], rot_range[1], rot_interval):
            for next_scale in range(scale_range[0], scale_range[1], scale_interval):
                scaled_template, actual_scale = scale_image(rgbtemplate, next_scale, image_maxwh)
                print("Angle " + str(next_angle) + "\t" + "Scale " + str(next_scale))
                if next_angle == 0:
                    rotated_template = scaled_template
                else:
                    rotated_template = rotate_image(scaled_template, next_angle)
...
def main():
    img_bgr = cv2.imread("Bilder/Szene/Szene1.jpg")
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    template_bgr = plt.imread("Bilder/Target/Taschenrechner.jpg")
    ...
    plt.show()
    points_list = modifiedMatchTemplate(img_rgb, cropped_template_rgb, "TM_CCOEFF_NORMED", 0.5, 100, [0,360], 10, [90,110], 2, True, True)
